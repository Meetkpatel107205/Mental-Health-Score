(() => {
  ...
  const API_BASE = "https://mental-health-score-85zu.onrender.com";
  ...
  function showState(name) {
    [stateIdle, stateLoading, stateResult].forEach((el) => (el.hidden = true));
    ({
      idle: stateIdle,
  ...
  form.addEventListener("submit", async (e) => {
    ...
    try {
      ...
      if (res.status === 422) {
        ...
        applyServerValidationErrors(body && body.detail);
        showState("idle");
        return;
      }
      ...
      console.error(`Couldn't connect to ${API_BASE}.`, err);
      showState("idle");
    } finally {
      setSubmitting(false);
    }
